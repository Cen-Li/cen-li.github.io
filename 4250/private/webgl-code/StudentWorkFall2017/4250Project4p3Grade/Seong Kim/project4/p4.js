...
function SetupTextures() {
    createTextures("Textures/White.png");       // 0
    createTextures("Textures/Plane.png");       // 1
    createTextures("Textures/Stars.png");       // 2
    createTextures("Textures/Skyscraper.png");  // 3
    createTextures("Textures/ObiliskBody.jpg"); // 4
    createTextures("Textures/Eye.jpg");         // 5
    createTextures("Textures/EyeLid.png");      // 6
    createTextures("Textures/Beams.png");       // 7
    createTextures("Textures/MainBeam.png");    // 8
    createTextures("Textures/ObiliskHand.jpg"); // 9
    createTextures("Textures/HandEye.jpg");     // 10
    createTextures("Textures/GameOver.png");    // 11
    createTextures("Textures/YouWin.png");      // 12
    createTextures("Textures/DoubleKill.png");  // 13
}

function createTextures(ImageLoc) {
    var i = textures.length;
    textures.push(gl.createTexture());
    textures[i].image = new Image();
    textures[i].image.src = ImageLoc;
    //textures[i].image.onload = function() { loadTexture(textures[i], window['gl.TEXTURE' + i])};
    textures[i].image.onload = function() { loadTexture(textures[i], gl.TEXTURE0 + i)};
    //textures[i].image.crossOrigin = "Anonymous"; //cross-origin issue fix
}

function stopSound(Ind) {
    sounds[Ind].pause();
...
function SetupSounds() {
    ...
    // 5 - Obilisk Beam 2
    // 6 - 
    // 7 - Obilisk Explosion 1
    // 8 - Obilisk Explosion 2
    // 9 - Player Win
    // 10 - Player Loose
    //sounds.push(new Audio("Sounds/bensound-scifi.mp3"));
    sounds.push(new Audio("Sounds/bensound-extremeaction.mp3"));
    sounds[0].volume = 0;
    ...
    sounds.push(new Audio("Sounds/pew.mp3"));
    sounds.push(new Audio("Sounds/pew2.mp3"));
    sounds[1].volume = 0.3;
    sounds[2].volume = 0.1;

    sounds.push(new Audio("Sounds/hit.mp3"));

    sounds.push(new Audio("Sounds/computerPowerDown.mp3"));
    sounds.push(new Audio("Sounds/creepyGamePro.mp3"));

    sounds.push(new Audio("Sounds/beam.mp3"));
    sounds[6].volume = 1.0;

    sounds.push(new Audio("Sounds/explosion01.mp3"));
    sounds.push(new Audio("Sounds/explosion02.mp3"));
    sounds[7].volume = 0.6;
    sounds[8].volume = 0.6;

    sounds.push(new Audio("Sounds/applause.mp3"));
    sounds.push(new Audio("Sounds/booing.mp3"));

}

function soundUpdate() {
    if(start && !started) {
        if(sounds[0].volume < 0.7)
            sounds[0].volume += 0.005;
    }

    if((!ObiliskAlive || !PlayerAlive) && sounds[0].volume > 0.005 ) {
        sounds[0].volume -= 0.005;
    }
}
...
function ResetGame() {
    ...
    //Obiliks body
    obLife = 20;
    tb = translate(0,-2.5,0);

    //Obilisk hand right
    orLife = 10;
    thr = translate(0,-3,0.1);
    rhr = -70;

    //Obilisk hand left
    olLife = 10; //health
    thl = translate(1.5,-3,0.1);
...
function GenerateTowers() {
    ...
    for(var i = 0; i < 8; i++) {
        towerCur.push(GetRandomNumber(0,n_tower));
        //towerCur.push(5);
    }
...
function DrawBackground() {
    ...
    materialDiffuse = vec4(255/255,1,255/255,1);
...
function WinLoseCard() {
    var s;
    var t;

  if(!PlayerAlive || !ObiliskAlive) {
        // lighting and material for towers
        materialAmbient = vec4( .2, .2, .2, 1.0 );
...
function DrawObiliskHealth() {
    ...
    if(eyeOpen) {
        ...
        materialAmbient = vec4( .2, .2, .2, 1.0 );
        materialDiffuse = vec4( 200/255, 0/255, 200/255, 1.0 );
        materialSpecular = vec4( 50/255, 50/255, 50/255, 1.0 );
        materialShiness=60;
        SetupLightingMaterial();
    }
    ...
    materialShiness=60;
    SetupLightingMaterial();
    t = addTrans(t, translate(0,-0.0044,0));
    ...
    //Hand health bars (Right)
    s=scale4(.1, .1, 0.5);
    t=translate(6, 3.53, 4.3);
    ...
    for(var i = 0; i < orLife; i ++) {
        ...
        gl.drawArrays(gl.TRIANGLES, 2160, 36); //36
        modelViewMatrix=mvMatrixStack.pop();
        t = addTrans(t, translate(0,0.0044,0.25));
    }
    ...
    //Hand health bars (Left)
    s=scale4(.1, .1, 0.5);
    t=translate(6, 3.49, 1);
...
function DrawObilisk() {
    ...
    if(ObiliskAlive) { //While Obilisk is Alive
        if(started) { //If it has started
        ...
        } else if (start && !started) { //starting
            if(tb[1][3] < 0) {
                tb[1][3] += 0.018;
            }
            //Right Hand
            ...
            if(tb[1][3] >= 0 && thr[1][3] >= 0.2 && thl[1][3] >= -0.1) {
                started = true;
            }
...
function UpdateTimedEvent() {
    clearTimedEvent();
    obLife; 20
    oBulletTimed = setInterval(bulletAttack,500 * GetRandomNumber(1,obLife/2));
    oBeamTimed = setInterval(beamAttack,1000 * GetRandomNumber(1, 10));
}
...
function beamAttack() {
    ...
    t_c = setTimeout(UpdateTimedEvent,4000);
    sounds[5].play();
}
...
function Death() {
    ...
    if(handL && olLife <= 0) {
        handL = false;
        t_rl = setTimeout(Revive,15000,"handL","olLife",10);
    }
    if(handR && orLife <= 0) {
        handR = false;
        t_rr = setTimeout(Revive,15000,"handR","orLife",10);
    }
...
function polygon(vertices)
{
    ...
    for (var i=0; i<M-2; i++)
    {
        ...
        normalsArray.push(normal);
        texCoordsArray.push(texCoord[1 + v]);
        ...
        normalsArray.push(normal);
        texCoordsArray.push(texCoord[2 + v]);
