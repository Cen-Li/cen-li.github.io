...
window.onload = function init() 
{
    ...
    document.getElementById("zoomIn").onclick=function(){zoomFactor *= 0.95;requestAnimFrame(render);};
    document.getElementById("zoomOut").onclick=function(){zoomFactor *= 1.05;requestAnimFrame(render);};
    document.getElementById("left").onclick=function(){translateFactorX -= 0.1;requestAnimFrame(render);};
    document.getElementById("right").onclick=function(){translateFactorX += 0.1;requestAnimFrame(render);};
    document.getElementById("up").onclick=function(){translateFactorY += 0.1;requestAnimFrame(render);};
    document.getElementById("down").onclick=function(){translateFactorY -= 0.1;requestAnimFrame(render);};
    // keyboard handle
...
function DrawWindowPanePiece() {
	// start using gl.TEXTURE4
    gl.uniform1i(gl.getUniformLocation(program, "texture"), 4);  // fragment shader to use gl.TEXTURE4
...
function DrawChair(topWid, topThick, legThick, legLen, backHeight) {
	...
	// Draw the chair back
	mvMatrixStack.push(modelViewMatrix);
	t=translate(0, legLen+topThick/2+(backHeight/2), topWid/2-topThick/2);
...
function DrawPlate() {
	...
	materialDiffuse = vec4( 211/255,211/255,230/211, 1.0);
	diffuseProduct = mult(lightDiffuse, materialDiffuse);
    gl.uniform4fv( gl.getUniformLocation(program, "diffuseProduct"),flatten(diffuseProduct) );
	
	mvMatrixStack.push(modelViewMatrix);
	//t=translate(-0.1, 0, 0.1);
    //modelViewMatrix=mult(modelViewMatrix, t);
	s = scale4(0.33, 0.33, 0.33);
	modelViewMatrix = mult(modelViewMatrix, s);
    gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(modelViewMatrix));
	gl.drawArrays( gl.TRIANGLES, plateBegin, plateLength);
	modelViewMatrix=mvMatrixStack.pop();
}
...
function Initialize_Textures()
{
	...
	//loadTexture
	texture2.image.src='woodfloor.jpg';
	...
	//loadTexture
	texture3.image.src='logcabin.jpg';
	...
	//loadTexture
	texture5.image.src='whitewood.jpg';
...
function GenerateWallWithWindow()
{
	...
	texCoord.push(vec2(0.3, 0.2));
	texCoord.push(vec2(0.7, 0.2));
	texCoord.push(vec2(0.7, 0.5));
	texCoord.push(vec2(0.3, 0.5));
...
function Plate() {
	plateBegin = pointsArray.length;
	var startVertex = vertices.length;
	var numPointsPerStep = 5;
	
	
	vertices.push(vec4(0, 0, 0, 1));
	vertices.push(vec4(0.25, 0, 0, 1));
	vertices.push(vec4(0.3, 0.03, 0, 1));
	vertices.push(vec4(0.25, 0.01, 0, 1));
	vertices.push(vec4(0, 0.01, 0, 1));
	
	var r;
	var t = Math.PI/12;
	
	// sweep the original shape
	for (var j = 0; j < 68; j++) {
		var angle = (j+1)*t;
		
		for (var i = 0; i < numPointsPerStep; i++) {
			r = vertices[i+startVertex][0];
			vertices.push(vec4(r*Math.cos(angle), vertices[i+startVertex][1], -r*Math.sin(angle), 1));
		}
	}
	
	// Form quad strips slice by slice
	for (var i = 0; i < numPointsPerStep; i++) {
		for (var j=0; j < 100; j++) {
			quad(i*numPointsPerStep+j+startVertex, (i+1)*numPointsPerStep+j+startVertex, (i+1)*numPointsPerStep+(j+1)+startVertex, i*numPointsPerStep+(j+1)+startVertex);
		}
	}
	
	
	plateLength = pointsArray.length - plateBegin;
}
