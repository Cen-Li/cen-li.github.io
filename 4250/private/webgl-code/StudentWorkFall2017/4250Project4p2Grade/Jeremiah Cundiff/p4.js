...
var Ground = {
    ...
    Draw: function () {
        ...
        modelViewMatrix = mult(modelViewMatrix, scale4(200, .25, 20));
        gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(modelViewMatrix));
...
var Cliff = {
    Generate: function () {
        generatePlane([3, 3, 3, 3, 3, 3], 36);
    },
    Draw: function () {
        modelViewStack.push(modelViewMatrix);
        modelViewMatrix = mult(modelViewMatrix, translate(0, -2, -15));
        modelViewMatrix = mult(modelViewMatrix, rotate(90, 90, 0, 1));
        modelViewMatrix = mult(modelViewMatrix, scale4(200, .25, 20));
        gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(modelViewMatrix));
        Scene.DrawObject("plane");
        modelViewMatrix = modelViewStack.pop();
