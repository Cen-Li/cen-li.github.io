...
function main() {
    ...
    var Radius = 0.7;
    var starPoints = GeneratePoints(center, radius, Radius);
	var circlePoints = GenerateCirclePoints(center, Radius);
	var points = starPoints.concat(circlePoints);
...
function GeneratePoints(center, radius, Radius)
{
	...
	for (var i=0; i<13; i++) {
		if (i % 2 == 0) {
			// inner
			vertices.push(vec2(center[0]+radius*Math.cos(i*angle),
...
function render() {
	...
	gl.drawArrays( gl.LINE_STRIP, 13, 101);
}
