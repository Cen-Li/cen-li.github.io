...
function GeneratePoints(center, radius, Radius)
{
    ...
    {
        // 3 points for the first triangle 
       ...
       // 3 points for the second triangle
       vertices.push(vec2(center[0], center[1]));
       vertices.push(vec2(center[0]+radius*Math.cos((i*angle)+Math.PI/4), center[1]+radius*Math.sin((i*angle)+Math.PI/4)));
...
function render() {
    ...
    {
        gl.uniform1i(gl.getUniformLocation(program, "colorIndex"), k%2+1); // colorIndex is 1 or 2
        gl.drawArrays( gl.TRIANGLES, k*3, 3);  // use TRIANGLE here
    }
}
