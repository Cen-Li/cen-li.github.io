...
function GeneratePrimitives()
{
    ...
    texCoord = [ vec2(0, .2), vec2(0, 0), vec2(.2, .2), vec2(.2, 0), ];
    GenerateSphere(radius, slices, stacks);    
    
	radius=0.4;
	height=1;
    texCoord = [ vec2(0, 1), vec2(0, 0), vec2(1, 1), vec2(1, 0), ];
...
function GenerateCone(radius, height)
{
    ...
    var textCod=[];
    var tc0=vec2(0.5, 1); // top texture coordinate
    var tc1=vec2(0, 0);   // bottom left  ''    ''
    var tc2=vec2(1, 0);   // bottom right ''    ''
    var t1, t2, t3, t4;
    ...
    for (var i=1; i<=slices; i++) {
        ...
        // texture coordinate
        var prev, cur;
        prev=tc1+(i-1)/slices*(tc2-tc1);
        cur =tc1 + i/slices*(tc2-tc1);
        ...
        // form texture coordinates for the triangle
        t1=tc0;
        t2=tc0+1/stacks*(prev-tc0);
        t3=tc0+1/stacks*(cur-tc0);
        
        // triangle bottom of the cone
        triangleS(prev[0], prev[1], curr[1], t1, t2, t3);
        ...
        for (var j=1; j<stacks; j++) {
            ...
            // form texture coordinates for the triangle
            t1=tc0+j*(1/stacks)*(prev-tc0);  // v1
            t2=tc0+(j+1)*(1/stacks)*(prev-tc0);  // v2
            t3=tc0+(j+1)*(1/stacks)*(cur-tc0); // v3
            t4=tc0+j*(1/stacks)*(cur-tc0); // v4;

            // form the quad
            quadS(prev1, curr1, curr2, prev2, t1, t2, t3, t4);
        }
...
// special texture: t is vec2 array
function triangleS(a, b, c, t1, t2, t3) 
{
    // triangle abc
   	pointsArray.push(a);
   	normalsArray.push(normalize(a));
    texCoordsArray.push(t1);

   	pointsArray.push(b);
   	normalsArray.push(normalize(b));
    texCoordsArray.push(t2);

   	pointsArray.push(c);
   	normalsArray.push(normalize(c));
    texCoordsArray.push(t3);
}
...
// a, b, c, and d are all vec4 type
// texture special: t is vec2 array
function quadS(a, b, c, d, t1, t2, t3, t4) 
{
    // triangle abc
   	pointsArray.push(a);
   	normalsArray.push(normalize(a));
