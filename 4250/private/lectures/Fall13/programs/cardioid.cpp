//
//  spiral.cpp
//  Spiral and Cardioid curves
//
//  Created by Cen Li on 9/3/12.
//
//
// Cardioid is drawn with the parametric curve
//         x = K * (1 + cos(theta)) * cos(theta) and 
...
void myinit()
{
	...
	glColor3f (0.0, 0.0, 1.0); 					//set the foreground color to black
...
void drawCardioid()
{
	...
	for (GLdouble theta = 0; theta <= 360.0; theta += 2.0)
	{
		...
		x = 1.0 * (1 + cos(radians)) * cos(radians);   
		y = 1.0 * (1 + cos(radians)) * sin(radians);
