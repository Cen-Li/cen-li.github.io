...
using namespace std;
Name ReadName();
// Pompts for and reads a name from the standard input device
int main()
{
  Name data[15];
  Name name; 
  ...
  // Sort 15 names
  QuickSort(data, 0, 14); 
  // Print first and last name in sorted order
  for (int index = 0; index < 15; index++)
    cout << data[index].GetFirstName() << " "
         << data[index].GetLastName() << endl; 
  return 0;
}
